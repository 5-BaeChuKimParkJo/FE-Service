...
- **실시간 경매 입찰**: 대용량 동시 입찰 처리를 가정해 단순 API 요청/응답 방식이 아니라, Server-Sent Events(SSE)를 활용하여 실시간으로 입찰 처리 결과를 이벤트 기반으로 받아 처리 및 커스텀 예외처리. 이를 통해 많은 사용자가 동시에 입찰해도 빠르고 안정적으로 각 사용자에게 개별 결과가 실시간으로 전달

- **실시간 채팅의 낙관적 UI**: 채팅 메시지 전송 시 서버 응답을 기다리지 않고, 사용자의 입력이 즉시 UI에 반영되는 낙관적 업데이트(Optimistic UI)를 적용하여 실시간성과 사용자 경험을 고려

- **UI/UX 디테일**: UI/UX 측면에서는 에러 상황이 발생하면 페이지별 error.tsx를 통해 사용자에게 에러 페이지를 제공하고, 페이지나 컴포넌트가 로딩될 때는 스켈레톤 UI로 자연스러운 전환을 구현. 입력 지연이나 불필요한 렌더링을 막기 위해 디바운스와 쓰로틀링을 적극 활용, template.tsx를 통해 공통 레이아웃을 관리하는 동시에 페이지 전환 시 애니메이션 효과 적용. 또한, framer-motion을 활용해 인터랙션에 자연스러운 애니메이션을 더해, 전체적으로 부드럽고 완성도 높은 사용자 경험을 제공하고자 함
