...
- **실시간 경매 입찰**: 대용량 데이터 처리를 위해 단순 API 요청/응답 방식이 아니라, Server-Sent Events(SSE)를 활용하여 실시간으로 입찰 정보를 스트리밍 방식으로 받아 처리 및 커스텀 예외처리. 이를 통해 많은 사용자가 동시에 입찰해도 빠르고 안정적으로 데이터가 반영
