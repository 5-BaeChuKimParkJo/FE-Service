...
export default function AuctionContent({ categories }: AuctionContentProps) {
  ...
  return (
    <>
      ...
      {isLoading && auctionProducts.length === 0 ? (
        ...
        <>
          {auctionProducts.length === 0 && !isLoading ? (
            <div className='text-center py-12'>
              <div className='text-gray-500 text-lg mb-2'>
                {categoryName
                  ? `${decodeURIComponent(categoryName)} 카테고리에 경매 상품이 없습니다`
                  : '경매 상품이 없습니다'}
              </div>
            ...
            <AuctionProductGrid
              products={auctionProducts.map((auction) => ({
      ...
      {!hasNextPage && !isLoadingMore && auctionProducts.length > 0 && (
        <div className='text-center py-8'>
          <p className='text-gray-500 text-sm'>
            {categoryName
              ? `${decodeURIComponent(categoryName)} 카테고리의 모든 경매를 불러왔습니다`
              : '모든 경매를 불러왔습니다'}
          </p>
        </div>
      )}
